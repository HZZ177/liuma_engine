...
class ApiRequestCollector:
    ...
    def collect_controller(self, api_data):
        if "saveWebsocket" not in api_data["controller"]:
            api_data["controller"]["saveWebsocket"] = None  # 默认不保存websocket
        if "clearMessageBefore" not in api_data["controller"]:
            api_data["controller"]["clearMessageBefore"] = "false"  # 默认执行前不清空消息
    ...
    def collect_conditions(self, api_data):
        self.conditions = api_data["controller"]

    def collect_looper(self, api_data):
        self.looper = api_data["controller"]

    def collect_variables(self, api_data):
        self.variables = api_data["controller"]

    def collect_script(self, api_data):
        self.script = api_data["controller"]
    ...
    def collect_rest(self, api_data):
        if len(api_data["rest"]) > 0:
            self.rest = api_data["rest"]
        else:
            self.rest = None
    ...
    def collect_message(self, api_data):
        body = api_data["body"]
        self.message_type = body["type"]
        self.message_text = body["text"]
    ...
    def collect(self, api_data):
        ...
        if self.apiId == 'looper':
            self.collect_looper(api_data)
        elif self.apiId == 'condition':
            self.collect_conditions(api_data)
        elif self.apiId == 'variable':
            self.collect_variables(api_data)
        elif self.apiId == 'script':
            self.collect_script(api_data)
        elif self.apiId == 'message':
            self.collect_message(api_data)
            self.collect_assertions(api_data)
            self.collect_relations(api_data)
            self.collect_controller(api_data)
        else:
            self.collect_protocol(api_data)
