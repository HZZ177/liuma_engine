...
class LMApi(Api):
    ...
    def apply_token(self):
        """"申请token"""
        url = self.url + "/autotest/engine/token/apply"
        data = {
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                if status == 0:
                    ...
                    DebugLogger("调用申请token接口 引擎code或秘钥错误")
                else:
    ...
    def fetch_task(self):
        """"获取任务"""
        url = self.url + "/autotest/engine/task/fetch"
    ...
    def upload_result(self, task_id, data_type, result):
        """"上传执行结果"""
        url = self.url + "/autotest/engine/result/upload"
        for index in range(2):
            data = {
                "engineCode": self.engine,
                "timestamp": int(time.time()),
    ...
    def complete_task(self, task_id):
        """"反馈任务结束"""
        url = self.url + "/autotest/engine/task/complete"
        for index in range(2):
            data = {
                "engineCode": self.engine,
                "timestamp": int(time.time()),
    ...
    def upload_screen_shot(self,task_image_path, uuid, log_path):
        """"上传执行截图"""
        url = self.url + "/autotest/engine/screenshot/upload"
        for index in range(2):
            data = {
                "fileName": "%s.png" % uuid,
                "engineCode": self.engine,
    ...
    def download_test_file(self, uuid):
        """下载测试文件"""
        url = self.url + "/autotest/engine/test/file/download/" + uuid
        for index in range(2):
            try:
                if index > 0:
